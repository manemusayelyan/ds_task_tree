...
/*
* խնդիր N19
* Գտնել տրված բինար ծառի մեկ որդի ունեցող հանգույցների քանակը։
*/
...
void preorder_iterative(t_node* root)
{
	...
	while (!st.empty())
	{
		...
		if ((cur_node->left && !cur_node->right) || (!cur_node->left && cur_node->right))
			count++;
		cout << cur_node->data << " ";
		st.pop();
	...
	cout << endl;
	cout << "Number of nodes with one child in given tree is " << count << endl;
}
